#!/bin/bash
# Waybar module: Claude Code rate-limit usage (5h session window + 7d window).
# Prints one line of JSON for a custom/* module with "return-type": "json".
# Empty text => waybar hides the module (same trick as recording.sh).
#
...
# The robot glyph's ink runs wider than the bell/updates glyphs, so a plain
# space leaves a 4px gap where those two show 6px. $pad is a pango-sized sliver
# of extra space that lines this module up with the rest of the right cluster.
pad='<span size="2560"> </span>'
text="󰚩 ${pad}${five_pct}%"
[ "$five_pct" -lt 0 ] && text="󰚩 ${pad}7d ${seven_pct}%"
