#!/bin/bash
# Claude Code statusLine hook for the quickshell agent-usage module.
#
...
#                   "command": "bash ~/.config/quickshell/scripts/agent-usage-statusline.sh" }
#
...
out="${XDG_CACHE_HOME:-$HOME/.cache}/quickshell/agent-usage-statusline.json"
...
if [ -s "$tmp" ]; then
  if cmp -s "$tmp" "$out"; then
    ...
    # The module keeps a FileView on this path, so replacing the file is
    # itself the notification — there is no signal to send.
    mv -f "$tmp" "$out"
  fi
else
  rm -f "$tmp"
